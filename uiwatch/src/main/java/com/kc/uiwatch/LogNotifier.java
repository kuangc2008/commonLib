...
public class LogNotifier {
     ...
     * 默认Log的获取间隔
     */
    private static long DEFAULT_DELAY = 30;
    ...
    private void handleLogNotifierMessage(Message msg) {
        ...
        switch (type) {
            ...
            case TYPE_OUTPUT:
                //通知LogExecutor处理输出任务
                sendOutputMessageToLogExecutor(msg.arg1);
                break;
            default:
                break;
        }
     ...
     */
    private void sendOutputMessageToLogExecutor(int diffTime) {
        if (logExecutor == null) {
            return;
        }
        Message message = logExecutor.obtainOutputMessage();
        message.arg1 = diffTime;
     ...
     * 开始输出并重启收集通知
     */
    public void startOutputAndRestartCollectionNotifier(int skipFrameCount) {
        removeAllMessage();
        sendOutputMessage(skipFrameCount, 0);
     ...
     * 发送输出消息
     */
    private void sendOutputMessage(int skipFrame, long delay) {
        if (logNotifierHandler == null) {
            return;
        }
        Message outputMessage = Message.obtain(logNotifierHandler, TYPE_OUTPUT);
        outputMessage.arg1 = skipFrame;
    ...
    public void setCacheDataSize(int cacheDataSize, int cacheDelayTime) {
        DEFAULT_DELAY = cacheDelayTime;
        if (logExecutor != null) {
            logExecutor.setCacheDataSize(cacheDataSize, cacheDelayTime);
        }
    }
    ...
    public void setListener(UiWatcher.BlockListener listener) {
        if (logExecutor != null) {
            logExecutor.setListener(listener);
        }
    }
}
